...
- **GEEN subpagina's-regel (Chris, 2026-07-09):** records openen áltijd in een slideover-paneel rechts, nooit in een Payload-subpagina. Panelen: `TaakPanel` (`?taak=<id|nieuw>&status=` op /admin/taken — velden + interactieve checklist met voortgangsmeter), `ContentPanel` (`?item=<id|nieuw>&datum=` op /admin/kalender — status-flow als klikbare stappen, brief; dagnummer-klik in de kalender = nieuw item op die dag), `DealPanel`, `DocPanel`. Nieuwe record-types volgen ditzelfde patroon; de Payload-editor blijft alleen als "Openen in volledige editor"-fallback.
