# Second Brain — bouwrapport

Structurele graphify-run, geen LLM (PRD §6.2). Automatisch gegenereerd door scripts/agent/build_graph.py — niet handmatig bewerken.

## Totalen
- Documenten gescand: 0
- Nodes: 290
- Edges: 418
- Clusters: 29

## Clusters
- **Roadmap Wensen** (42 nodes)
- **Agents** (34 nodes)
- **Roadmap Wensen** (32 nodes)
- **Modules Data** (25 nodes)
- **Modules Data** (22 nodes)
- **Platform Wiki** (19 nodes)
- **Roadmap Wensen** (17 nodes)
- **Platform Wiki** (9 nodes)
- **Modules Data** (8 nodes)
- **Modules Data** (8 nodes)
- **Roadmap Wensen** (8 nodes)
- **Platform Wiki** (7 nodes)
- **Agent Skills SOP S** (6 nodes)
- **Werkbladen** (6 nodes)
- **Werkbladen** (6 nodes)
- **Werkbladen** (6 nodes)
- **Werkbladen** (6 nodes)
- **Werkbladen** (6 nodes)
- **Platform Wiki** (5 nodes)
- **Agent Skills SOP S** (4 nodes)
- **Handleiding Dashboard** (4 nodes)
- **Handleiding Dashboard** (2 nodes)
- **Werkwijze Human Margin** (2 nodes)
- **Algemeen** (1 nodes)
- **Algemeen** (1 nodes)
- **Handleiding Dashboard** (1 nodes)
- **Handleiding Dashboard** (1 nodes)
- **Handleiding Dashboard** (1 nodes)
- **Handleiding Dashboard** (1 nodes)

## Meest verbonden (god nodes)
- De vier operaties — graad 5
- De loop — graad 1
- Waarom loggen — graad 1
- Spelregels — graad 1
- Naam en trigger — graad 1
- Context vooraf verzamelen — graad 1
- Vragen die altijd eerst gesteld worden — graad 1
- Stappen — graad 1
- Definition of done — graad 1
- Het pipeline-board — graad 1

## Verrassende verbindingen
- (geen)
